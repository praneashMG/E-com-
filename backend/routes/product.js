...
const { getProducts, newProduct, getSingleProduct, updateProduct, deleteProduct, createReview, getReviews, deleteReview, getAdminProducts, newMobile, getProductsByCategory } = require('../controllers/productController');
...
router.route("/product/category").post(getProductsByCategory)
                      


//Admin routes
router.route('/admin/product/new').post(isAuthenticatedUser, authorizeRoles('admin'), upload.array('images'), newProduct);
router.route('/admin/mobile/new').post(isAuthenticatedUser, authorizeRoles('admin'), upload.array('images'), newMobile);
